...
# 2. Add MapController to MapContainer
map_container = """<MapContainer 
                  center={newBranchData.latitude && newBranchData.longitude ? [parseFloat(newBranchData.latitude), parseFloat(newBranchData.longitude)] : [4.2248, 103.4194]} 
                  zoom={16} 
                  style={{ height: "100%", width: "100%" }}
                >
                  <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                  <MapController center={newBranchData.latitude && newBranchData.longitude ? [parseFloat(newBranchData.latitude), parseFloat(newBranchData.longitude)] : [4.2248, 103.4194]} />
...
new_button = """<Button type="button" variant="outline" className="w-full h-10 rounded-xl text-xs font-bold text-purple-700 bg-purple-50 hover:bg-purple-100 border-purple-200" onClick={() => {
                      if (newBranchData.latitude && newBranchData.longitude) {
                        const lat = parseFloat(newBranchData.latitude);
                        const lng = parseFloat(newBranchData.longitude);
                        fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`)
                          .then(res => res.json())
                          .then(data => {
                            if(data && data.display_name) {
                              setNewBranchData(prev => ({...prev, location: data.display_name}));
                            }
                          }).catch(console.error);
                      }
                    }}>
