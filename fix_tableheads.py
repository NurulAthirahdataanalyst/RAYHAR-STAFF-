...
def clean_classnames(content):
    def replace_tablehead(match):
        inner = match.group(1)
        # Remove font sizes
        inner = re.sub(r'\btext-(xs|sm|base|lg|xl|2xl|3xl)\b', '', inner)
        # Remove text colors (except text-white which was explicitly requested for purple backgrounds)
        inner = re.sub(r'\btext-(slate|gray|zinc|neutral|stone|indigo)-[0-9]+\b', '', inner)
        inner = re.sub(r'\bdark:text-(slate|gray|zinc|neutral|stone|indigo)-[0-9]+\b', '', inner)
