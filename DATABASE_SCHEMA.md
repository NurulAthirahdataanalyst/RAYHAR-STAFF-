...
> **System:** Rayhar Attendance & Leave Tracking Portal  
> **Database Engine:** PostgreSQL (Supabase)  
> **Total Tables:** 21 Tables  
> **Live Synchronized Date:** 13 Sept 2026

---

## 1. Entity-Relationship Overview

The database uses a **Hub & Spoke architecture** centered around the `profiles` table (`user_id`).

```mermaid
erDiagram
    branches ||--o{ profiles : "branch -> code"
    department ||--o{ profiles : "department -> name"
    role ||--o{ profiles : "role -> name"

    department ||--o{ user_role : "department -> name"
    role ||--o{ user_role : "role -> name"
    profiles ||--o{ user_role : "user_id"

    profiles ||--o{ attendances : "user_id"
    attendances ||--o{ employee_location_logs : "attendance_id"
    profiles ||--o{ employee_location_logs : "user_id"

    profiles ||--o{ outstation_assignments : "user_id"

    profiles ||--o{ leave_requests : "user_id"
    leave_requests ||--o{ leave_approvals : "leave_id"
    profiles ||--o{ leave_approvals : "approver_id -> user_id"
    leave_requests ||--o{ replacement_leave_requests : "leave_request_id -> leave_id"
    profiles ||--o{ leave_balance_adjustments : "employee_id -> user_id"

    profiles ||--o{ notifications : "user_id"
    profiles ||--o{ employee_allowed_locations : "user_id"
    profiles ||--o{ employee_work_assignment : "user_id"
```

---

## 2. Table Summary & Primary Keys

| # | Table Name | Primary Key (PK) | Data Type | Key Relationships |
|---|---|---|---|---|
| **1** | `activity_logs` | `id` | `integer` | - |
| **2** | `alerts` | `id` | `integer` | - |
| **3** | `attendances` | `attendance_id` | `integer` | `user_id` -> `profiles.user_id` |
| **4** | `branches` | `code` | `character varying` | - |
| **5** | `company_leave_calendar` | `id` | `integer` | - |
| **6** | `department` | `id` | `integer` | - |
| **7** | `employee_allowed_locations` | `id` | `integer` | `user_id` -> `profiles.user_id` |
| **8** | `employee_location_logs` | `id` | `integer` | `attendance_id` -> `attendances.attendance_id`<br>`user_id` -> `profiles.user_id` |
| **9** | `employee_work_assignment` | `id` | `integer` | `user_id` -> `profiles.user_id` |
| **10** | `hod_history` | `id` | `integer` | - |
| **11** | `leave_approvals` | `id` | `integer` | `approver_id` -> `profiles.user_id`<br>`leave_id` -> `leave_requests.leave_id` |
| **12** | `leave_balance_adjustments` | `id` | `integer` | `employee_id` -> `profiles.user_id` |
| **13** | `leave_requests` | `leave_id` | `integer` | `user_id` -> `profiles.user_id` |
| **14** | `notifications` | `id` | `integer` | `user_id` -> `profiles.user_id` |
| **15** | `outstation_assignments` | `id` | `integer` | `user_id` -> `profiles.user_id` |
| **16** | `personal_notes` | `id` | `integer` | - |
| **17** | `profiles` | `user_id` | `character varying` | `branch` -> `branches.code`<br>`department` -> `department.name`<br>`role` -> `role.name` |
| **18** | `replacement_leave_requests` | `id` | `bigint` | `leave_request_id` -> `leave_requests.leave_id` |
| **19** | `role` | `id` | `uuid` | - |
| **20** | `system_settings` | `setting_key` | `character varying` | - |
| **21** | `user_role` | `id` | `integer` | `department` -> `department.name`<br>`role` -> `role.name`<br>`user_id` -> `profiles.user_id`<br>`user_id` -> `profiles.user_id` |

---

## 3. Active Foreign Key Constraints (Live in Supabase)

| Source Table | Foreign Key Column | Target Table | Target Column | Update Rule | Delete Rule |
|---|---|---|---|---|---|
| `attendances` | `user_id` | `profiles` | `user_id` | `NO ACTION` | `CASCADE` |
| `employee_allowed_locations` | `user_id` | `profiles` | `user_id` | `NO ACTION` | `CASCADE` |
| `employee_location_logs` | `attendance_id` | `attendances` | `attendance_id` | `CASCADE` | `SET NULL` |
| `employee_location_logs` | `user_id` | `profiles` | `user_id` | `CASCADE` | `CASCADE` |
| `employee_work_assignment` | `user_id` | `profiles` | `user_id` | `NO ACTION` | `CASCADE` |
| `leave_approvals` | `approver_id` | `profiles` | `user_id` | `NO ACTION` | `CASCADE` |
| `leave_approvals` | `leave_id` | `leave_requests` | `leave_id` | `NO ACTION` | `CASCADE` |
| `leave_balance_adjustments` | `employee_id` | `profiles` | `user_id` | `NO ACTION` | `CASCADE` |
| `leave_requests` | `user_id` | `profiles` | `user_id` | `NO ACTION` | `CASCADE` |
| `notifications` | `user_id` | `profiles` | `user_id` | `NO ACTION` | `CASCADE` |
| `outstation_assignments` | `user_id` | `profiles` | `user_id` | `CASCADE` | `CASCADE` |
| `profiles` | `branch` | `branches` | `code` | `NO ACTION` | `SET NULL` |
| `profiles` | `department` | `department` | `name` | `CASCADE` | `SET NULL` |
| `profiles` | `role` | `role` | `name` | `CASCADE` | `SET NULL` |
| `replacement_leave_requests` | `leave_request_id` | `leave_requests` | `leave_id` | `NO ACTION` | `CASCADE` |
| `user_role` | `department` | `department` | `name` | `CASCADE` | `SET NULL` |
| `user_role` | `role` | `role` | `name` | `CASCADE` | `SET NULL` |
| `user_role` | `user_id` | `profiles` | `user_id` | `NO ACTION` | `CASCADE` |

---

## 4. Full SQL Schema Script

The full SQL schema has been exported to [`supabase_schema.sql`](file:///c:/Users/HP/ATTENDANCE_SYSTEM/supabase_schema.sql).
You can run this script directly in any PostgreSQL instance or Supabase SQL Editor to recreate the entire database structure.
