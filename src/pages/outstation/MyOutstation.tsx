        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-5 pt-2">
              <div className="space-y-3">
                <p className="text-[10px] font-black uppercase tracking-widest text-foreground dark:text-gray-400 flex items-center gap-1.5 border-b border-gray-100 dark:border-slate-800 pb-1">
                  <MapPin className="w-3.5 h-3.5" /> Trip Details
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] font-bold text-foreground dark:text-gray-300 mb-1 block">Destination</label>
                    <Input readOnly value={viewFormAssignment.destination || ""} className="h-8 text-xs bg-muted/50 cursor-default" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-foreground dark:text-gray-300 mb-1 block">Client / Company</label>
                    <Input readOnly value={viewFormAssignment.client_company || ""} className="h-8 text-xs bg-muted/50 cursor-default" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-foreground dark:text-gray-300 mb-1 block">Event Name</label>
                    <Input readOnly value={viewFormAssignment.project || ""} className="h-8 text-xs bg-muted/50 cursor-default" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-foreground dark:text-gray-300 mb-1 block">Meeting Title</label>
                    <Input readOnly value={viewFormAssignment.meeting_title || ""} className="h-8 text-xs bg-muted/50 cursor-default" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="text-[10px] font-bold text-foreground dark:text-gray-300 mb-1 block">Purpose</label>
                    <textarea
                      readOnly
                      value={viewFormAssignment.purpose || ""}
                      rows={2}
                      className="w-full rounded-md border border-input bg-muted/50 px-3 py-2 text-xs cursor-default resize-none"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] font-bold text-foreground dark:text-gray-300 mb-1 block">Start Date</label>
                    <Input readOnly value={fmtDate(viewFormAssignment.start_date)} className="h-8 text-xs bg-muted/50 cursor-default" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-foreground dark:text-gray-300 mb-1 block">End Date</label>
                    <Input readOnly value={fmtDate(viewFormAssignment.end_date)} className="h-8 text-xs bg-muted/50 cursor-default" />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-muted/50 rounded-lg p-3 border border-border">
                  <p className="text-sm font-bold">{viewFormAssignment.full_name}</p>
                  <p className="text-xs text-muted-foreground">{viewFormAssignment.department} - {viewFormAssignment.branch}</p>
                  <div className="mt-2">{statusBadge(viewFormAssignment.status)}</div>
                </div>
              </div>
            </div>

            <div className="flex justify-end pt-4 border-t border-gray-100 dark:border-slate-800 mt-6 gap-2">
              <Button variant="outline" size="sm" onClick={() => setViewFormAssignment(null)}>
                Close
              </Button>
            </div>
            </>
