...
export default function OutstationAnalytics() {
  ...
  const kpis = [
    ...
    { label: "TOP TRAVELER", value: stats.topTraveler ? stats.topTraveler[0].split(" ").slice(0, 2).join(" ") : "—", icon: Medal, color: "text-purple-800", border: "border-l-[4px] border-l-green-500", sub: stats.topTraveler ? stats.topTraveler[1] : "—", isTrend: false },
    { label: "TOP DESTINATION", value: stats.topDest ? stats.topDest[0] : "—", icon: MapPin, color: "text-blue-600", border: "border-l-[4px] border-l-cyan-500", sub: "HQ Distribution Center", isTrend: false },
  ];
          ...
          return (
            <Card key={k.label} className={`border-0 shadow-sm ${k.border} rounded-md bg-white`}>
              <CardContent className="p-5 flex flex-col justify-between h-full">
                ...
                <div>
                  <p className={`text-3xl font-light ${k.color} leading-tight mb-2`}>{loading ? "—" : k.value}</p>
