  ...
  return (
    <div className="space-y-4 sm:space-y-4 animate-in fade-in duration-500">
      ...
      <div className="flex flex-row items-center gap-2 bg-card/50 backdrop-blur-sm p-3 rounded-2xl border border-border/50 overflow-x-auto">
        <div className="flex flex-row items-center gap-2 flex-1 min-w-0">
              ...
              <PopoverContent className="w-[340px] p-0 shadow-xl" align="start" onOpenAutoFocus={(e) => e.preventDefault()}>
                  ...
                  <div className="p-3 border-b border-border/50">
                    <div className="flex flex-wrap gap-1.5">
                      {checkedEmployees.map(id => {
                        ...
                        return emp ? (
                          <span key={id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#942392]/10 text-[#942392] text-[10px] print:text-[13px] font-bold">
                            {emp.name}
                        ...
                        {empList.map(emp => {
                          ...
                          return (
                            <div
                              ...
                              className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors ${isChecked ? 'bg-[#942392]/5' : 'hover:bg-yellow-50 dark:hover:bg-yellow-900/20'}`}
                            >
                              <label className="relative cursor-pointer" style={{width:18,height:18}} onClick={(e) => e.preventDefault()}>
                              ...
                              <div className="flex-1 min-w-0">
                                <p className={`text-xs print:text-[11px] font-bold truncate ${isChecked ? 'text-[#942392]' : 'text-foreground'}`}>{emp.name}</p>
                                <p className="text-[8px] print:text-[13px] text-muted-foreground truncate">{emp.user_id || emp.email || ''} · {emp.branch || ''}</p>
                              </div>
                              {isChecked && <span className="text-[10px] print:text-[13px] font-bold text-[#942392] bg-[#942392]/10 px-2 py-0.5 rounded-full">Selected</span>}
                            </div>
                          );
                        })}
                      </>
                ...
                {checkedEmployees.length > 0 && (
                  <div className="border-t border-border/50 p-2 flex justify-between items-center">
                    <span className="text-[10px] print:text-[13px] font-bold text-[#942392]">{checkedEmployees.length} selected</span>
                    <Button size="sm" variant="ghost" className="text-[8px] print:text-[13px] h-6 text-red-500 hover:text-red-600" onClick={() => setCheckedEmployees([])}>Clear All</Button>
                  </div>
                )}
              </PopoverContent>
            </Popover>
          ...
          {(["hr_admin", "managing_director", "operation_manager", "finance_manager"].includes(role) || uniqueBranches.length > 1) && (
            <Select value={selectedBranch} onValueChange={setSelectedBranch}>
              ...
              <SelectContent className="rounded-xl">
                <SelectItem value="All" className="text-xs print:text-[11px] font-bold">All Branches</SelectItem>
                {(Array.isArray(uniqueBranches) ? uniqueBranches : []).map((br) => (
                  <SelectItem key={br} value={br} className="text-xs print:text-[11px] font-bold">
          ...
          {(["hr_admin", "managing_director", "operation_manager", "finance_manager"].includes(role) || uniqueDepartments.length > 1) && (
            <Select value={selectedDepartment} onValueChange={setSelectedDepartment}>
              ...
              <SelectContent className="rounded-xl">
                <SelectItem value="All" className="text-xs print:text-[11px] font-bold">All Departments</SelectItem>
                {(Array.isArray(uniqueDepartments) ? uniqueDepartments : []).map((dept) => (
                  <SelectItem key={dept} value={dept} className="text-xs print:text-[11px] font-bold">
                    {toProperCase(dept)}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <Select value={selectedPosition} onValueChange={setSelectedPosition}>
            ...
            <SelectContent className="rounded-xl">
              <SelectItem value="All" className="text-xs print:text-[11px] font-bold">All Positions</SelectItem>
              {(Array.isArray(uniquePositions) ? uniquePositions : []).map((pos) => (
                <SelectItem key={pos} value={pos} className="text-xs print:text-[11px] font-bold capitalize">
                  {pos.replace(/_/g, ' ')}
                </SelectItem>
              ))}
            </SelectContent>
          ...
          <Select value={selectedStatus} onValueChange={setSelectedStatus}>
            ...
            <SelectContent className="rounded-xl">
              <SelectItem value="All" className="text-xs print:text-[11px] font-bold">All Statuses</SelectItem>
              <SelectItem value="Active" className="text-xs print:text-[11px] font-bold">Active Only</SelectItem>
              <SelectItem value="Inactive" className="text-xs print:text-[11px] font-bold">Inactive Only</SelectItem>
              <SelectItem value="Deleted" className="text-xs print:text-[11px] font-bold">Deleted Only</SelectItem>
            </SelectContent>
          </Select>
        ...
        <Badge variant="outline" className="px-3 py-1.5 text-xs print:text-[11px] font-bold whitespace-nowrap bg-muted/30 border-border/60 h-9 flex items-center justify-center rounded-md shrink-0">
          Total <span className="ml-2 flex items-center justify-center bg-[#942392] text-white rounded-md h-5 min-w-[20px] px-1.5 text-[8px] print:text-[13px] leading-none shrink-0">{filtered.length}</span>
        </Badge>
      </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            <div className="flex flex-col items-center justify-center p-12 gap-3">
              ...
              <p className="text-xs print:text-[11px] font-bold text-foreground animate-pulse uppercase tracking-widest">Loading Personnel...</p>
            </div>
          ) : (
            <>
              {/* Desktop Table View */}
                          ...
                          <TableCell className="py-4 px-6">
                            <div className="flex items-center gap-4">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-[8px] print:text-[13px] text-foreground truncate font-medium">{emp.email}</p>
                              </div>
                            </div>
                          </TableCell>
                          <TableCell className="py-4 px-6">
                            <span className="text-xs print:text-[11px] font-bold text-foreground capitalize">
                              {emp.position === "Finance Manager" || emp.position === "finance_manager" ? "Operation Manager" : emp.position.replace(/_/g, ' ')}
                            </span>
                          </TableCell>
                          <TableCell className="py-4 px-6 text-xs print:text-[11px] font-bold text-foreground">
                            <div className="flex flex-col gap-1 items-start">
                              <span>{emp.branch}</span>
                              {emp.tempBranch && (
                                <Badge variant="outline" className="text-[9px] print:text-[13px] bg-[#a01497]/10 text-[#a01497] border-[#a01497]/20 whitespace-nowrap">
                                  Temp: {emp.tempBranch}
                                </Badge>
                              )}
                            </div>
                  ...
                  (Array.isArray(currentItems) ? currentItems : []).map((emp) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-[10px] print:text-[13px] font-bold text-foreground uppercase tracking-wider">
                          ...
                          {emp.tempBranch && (
                            <>
                              <span className="opacity-30">â€¢</span>
                              <Badge variant="outline" className="text-[9px] print:text-[13px] bg-[#a01497]/10 text-[#a01497] border-[#a01497]/20">
                                Temp: {emp.tempBranch}
                              </Badge>
                            </>
                          )}
              ...
              {filtered.length > 0 && (
                <div className="flex flex-col sm:flex-row items-center justify-between p-4 border-t border-gray-100 dark:border-slate-800 gap-4 bg-slate-50/50 dark:bg-slate-900/50">
                  <div className="flex items-center gap-4 text-[10px] print:text-[13px] font-bold text-foreground uppercase tracking-widest">
                    <span>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <SelectTrigger className="h-7 text-xs print:text-[11px] font-bold rounded-lg border-border w-[70px]">
                  ...
                  <div className="flex items-center gap-1">
                    <Button 
                      ...
                      className="h-8 px-3 text-xs print:text-[11px] font-bold"
                    >
                      {"<"}
                    </Button>
                    <div className="flex items-center gap-1 overflow-x-auto max-w-[150px] sm:max-w-none scrollbar-hide">
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map(pageNum => (
                        <Button
                          ...
                          className={`h-8 w-8 p-0 text-xs print:text-[11px] font-bold ${currentPage === pageNum ? 'bg-[#a01497] text-white hover:bg-[#8a1182]' : 'text-slate-600 dark:text-slate-300'}`}
                        >
                          {pageNum}
                        </Button>
                      ))}
                            ...
                            <TableRow key={req.leave_id} className="cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => setSelectedLeaveFormDetail(req)}>
                              ...
                              <TableCell className="text-right">
                                <Button
                                  variant="outline"
                                  size="sm"
                                  className="h-7 text-[8px] print:text-[13px] bg-[#942392] text-white hover:bg-[#5c0073] font-bold"
                                  onClick={(e) => {
                                    e.stopPropagation();
