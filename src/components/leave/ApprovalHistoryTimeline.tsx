            ...
            <div key={index} className="relative flex items-center group">
              ...
              <div className="w-full bg-gray-50/80 dark:bg-slate-900/60 border border-gray-200/80 dark:border-slate-800 rounded-[18px] p-3 sm:px-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2 shadow-sm transition-all hover:bg-gray-50 dark:hover:bg-slate-900">
                <div className="flex items-center gap-3 flex-wrap">
                  {/* Status Badge */}
                  {isApproved && (
                    <span className="px-2.5 py-1 text-[10px] print:text-[11px] font-black uppercase tracking-wider rounded-md bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300">
                      APPROVED
                    </span>
                  )}
                  {isRejected && (
                    <span className="px-2.5 py-1 text-[10px] print:text-[11px] font-black uppercase tracking-wider rounded-md bg-rose-100 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300">
                      REJECTED
                    </span>
                  )}
                  {isPending && (
                    <span className="px-2.5 py-1 text-[10px] print:text-[11px] font-black uppercase tracking-wider rounded-md bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300">
                      PENDING
                    </span>
                  )}
  ...
  return (
    <div className={`flex flex-col sm:flex-row items-center ${(!showTotal && !showPageSize) ? 'justify-end' : 'justify-between'} p-4 border-t border-gray-100 dark:border-slate-800 gap-4 bg-slate-50/50 dark:bg-slate-900/50 ${className}`}>
      {(showTotal || showPageSize) && (
        <div className="flex items-center gap-4 text-[8px] print:text-[10px] font-bold text-foreground uppercase tracking-widest flex-wrap">
          {showTotal && (
            <span>
          ...
          {showPageSize && (
            <div className="flex items-center gap-2">
              ...
              >
                <SelectTrigger className="h-7 text-[8px] print:text-[10px] font-bold rounded border-border w-[65px] bg-card">
        ...
        {getPageNumbers().map((page, idx) => {
          ...
          return (
            <Button
              ...
              size="sm"
              className={`h-7 min-w-[28px] px-2 rounded-md text-[9px] print:text-[11px] font-bold ${
                isActive
