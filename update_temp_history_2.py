...
def fix():
    ...
    # Find the Temporary History Card block
    start_str = "{/* Temporary History Card */}"
    start_idx = text.find(start_str)
    
    # We want to find where the grid column ends, or where BOTTOM PANEL begins.
    # The block ends right before </div></div></div> \n\n {/* BOTTOM PANEL: ... */}
    bottom_panel_str = "{/* BOTTOM PANEL: Employee Attendance Data Table */}"
    bottom_idx = text.find(bottom_panel_str)
    
    # The actual card `</div>` closes around 2 levels deep from `bottom_idx`
    # Let's just use regex to replace from start_str to the last </div> before bottom_panel_str
    
    block_end_regex = re.compile(r'\n\s*\<\/div\>\n\s*\<\/div\>\n\s*\<\/div\>\n\n\s*\{\/\* BOTTOM PANEL: Employee Attendance Data Table \*\/\}')
    match = block_end_regex.search(text, start_idx)
    
    if not match:
        print("Could not find end of Temporary History Card")
        return
    ...
    # We will replace from start_idx to end_idx with active_assignment_code
    new_text = text[:start_idx] + active_assignment_code + text[end_idx:]
