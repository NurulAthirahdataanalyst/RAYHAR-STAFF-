...
# 1. Remove the top-level filters.
# They start after `<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-0">`
# or `<div className="mb-4">`
start_idx = content.find('<div className="mb-4">')
end_idx = content.find('<div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">', start_idx)

if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + content[end_idx:]

# 2. Add ABSENT to the status list and its filter logic.
# Line: `if (statusFilter === "ON TIME") { ... } else if (statusFilter === "LATE") { ... }`
status_logic = """if (statusFilter === "ON TIME") {
...
new_status_logic = """if (statusFilter === "ON TIME") {
    filteredList = filteredList.filter(e => e.status === "Present" && e.late === "00:00");
  } else if (statusFilter === "LATE") {
    filteredList = filteredList.filter(e => e.status === "Present" && e.late !== "00:00" && e.late !== "--");
...
content = content.replace(status_logic, new_status_logic)

# Replace the status map
content = content.replace('["ALL", "ON TIME", "LATE"].map', '["ALL", "ON TIME", "LATE", "ABSENT"].map')

# 3. Rewrite the CardHeader
header_start = content.find('<CardHeader className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">')
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated TeamAttendance.tsx")
